...
// --- DEFINIÇÃO DOS MODELOS (BASE DE DADOS) ---
...
// Cambistas
const Cambista = mongoose.model('Cambista', new mongoose.Schema({
    usuario: { type: String, required: true, unique: true },
    senha: { type: String, required: true },
...
// NOVOS MODELOS PARA O SISTEMA DE JOGADORES (CARTEIRA)
const JogadorSchema = new mongoose.Schema({
    nome: { type: String, required: true },
    telefone: { type: String, required: true, unique: true }, // Usado como Login
    senha: { type: String, required: true },
...
const TransacaoJogadorSchema = new mongoose.Schema({
    ...
    tipo: { type: String, required: true }, // 'deposito', 'compra', 'premio'
...
// Vendas (Agora suporta venda via Saldo e Jogador Logado)
const VendaSchema = new mongoose.Schema({
    ...
    tipo_venda: { type: String, required: true }, // 'Online', 'Manual', 'Cambista', 'Saldo'
    ...
    jogador_id: { type: mongoose.Schema.Types.ObjectId, ref: 'Jogador' }, // Link opcional
    timestamp: { type: Date, default: Date.now }
});
...
// Histórico Créditos Cambista
const TransacaoCredito = mongoose.model('TransacaoCredito', new mongoose.Schema({
    cambista_id: { type: mongoose.Schema.Types.ObjectId, ref: 'Cambista', required: true },
...
const PagamentoPendente = mongoose.model('PagamentoPendente', new mongoose.Schema({
...
// Inicialização de Dados Padrão (Admin e Configs)
...
// Middleware de Sessão (MongoDB Store)
app.use(session({
...
// --- WEBHOOK MERCADO PAGO (LÓGICA HÍBRIDA: VENDA DIRETA + DEPÓSITO) ---
    ...
    try {
        ...
        if (MERCADOPAGO_WEBHOOK_SECRET && signature && requestId && reqBody.data && reqBody.data.id) {
            ...
            if (parts.ts && parts.v1) {
                ...
                if (hmac.digest('hex') !== parts.v1) return res.sendStatus(403);
            }
        }
        ...
        if (reqBody.type === 'payment') {
            ...
            try {
                ...
                if (pag.status === 'approved') {
                    ...
                    if (pendente) {
                        const dados = JSON.parse(pendente.dados_compra_json);
                        
                        // LÓGICA 1: DEPÓSITO EM CARTEIRA
                        if (dados.tipo_transacao === 'deposito_carteira') {
                            const jogador = await Jogador.findById(dados.jogador_id);
                            if (jogador) {
                                jogador.saldo += parseFloat(dados.valor);
                                await jogador.save();
                                ...
                                console.log(`Depósito de R$${dados.valor} confirmado para ${jogador.nome}`);
                            }
                        } 
                        // LÓGICA 2: COMPRA DE CARTELA DIRETA (Visitante ou Logado sem saldo)
                        else {
                            ...
                            let vendaData = {
                                ...
                                quantidade_cartelas: dados.quantidade,
                                tipo_venda: 'Online',
                            ...
                            if (dados.tipo_compra === 'especial') {
                                ...
                                for(let i=0; i<dados.quantidade; i++) cartelasGeradas.push(gerarDadosCartela(sorteioAlvo));
                            }
                            ...
                            console.log(`Venda Direta #${novaVenda._id} confirmada via Webhook.`);
                ...
                console.error("Erro processando pagamento no MP:", errPayment);
            }
        }
        res.sendStatus(200);
    } catch (e) {
...
app.post('/api/player/register', async (req, res) => {
    ...
    try {
        ...
        if(existe) return res.status(400).json({success:false, message: 'Este telefone já está cadastrado.'});
        
        const hash = await bcrypt.hash(senha, 10);
        const novo = await Jogador.create({ nome, telefone, senha: hash });
        
        req.session.isPlayer = true;
        req.session.playerId = novo._id;
...
app.post('/api/player/login', async (req, res) => {
    ...
    try {
        const p = await Jogador.findOne({ telefone });
        if(p && await bcrypt.compare(senha, p.senha)) {
            req.session.isPlayer = true;
            req.session.playerId = p._id;
            res.json({success:true});
        } else {
            res.status(401).json({success:false, message: 'Telefone ou senha incorretos.'});
        }
    } catch(e) { 
...
// Logout
app.get('/api/player/logout', (req, res) => {
    req.session.destroy();
    res.redirect('/index.html');
});

// Dados do Jogador (Saldo e Histórico)
app.get('/api/player/me', async (req, res) => {
    if(!req.session.isPlayer) return res.status(401).json({success:false});
    try {
        const p = await Jogador.findById(req.session.playerId).select('-senha');
        const transacoes = await TransacaoJogador.find({ jogador_id: p._id }).sort({timestamp: -1}).limit(20);
        res.json({ success: true, jogador: p, transacoes });
    } catch(e) { 
...
// ==========================================================
// --- ROTAS DO ADMINISTRADOR (MIGRADO PARA MONGO) ---
// ==========================================================
...
app.get('/admin/premios-e-preco', checkAdmin, async (req, res) => {
    const configs = await Config.find({});
    const map = {}; configs.forEach(c => map[c.chave] = c.valor);
    res.json(map);
});
...
app.post('/admin/premios-e-preco', checkAdmin, async (req, res) => {
    ...
    try {
        ...
        const map = {}; novasConfigs.forEach(c => map[c.chave] = c.valor);
...
app.post('/admin/gerar-cartelas', checkAdmin, async (req, res) => {
    ...
    const preco = parseFloat(PRECO_CARTELA);
    const cartelas = []; for(let i=0; i<quantidade; i++) cartelas.push(gerarDadosCartela(sorteioAlvo));
    
    await Venda.create({
        sorteio_id: sorteioAlvo, nome_jogador: nome, telefone: telefone, quantidade_cartelas: quantidade,
        valor_total: quantidade * preco, tipo_venda: 'Manual', cartelas_json: JSON.stringify(cartelas), tipo_sorteio: 'regular'
    });
...
app.get('/admin/api/vendas', checkAdmin, async (req, res) => {
    ...
    const formatadas = vendas.map(v => ({
        ...v.toObject(), data_formatada: formatarDataBR(v.timestamp),
...
app.get('/admin/api/vencedores', checkAdmin, async (req, res) => {
    ...
    const formatados = vencedores.map(v => ({ ...v.toObject(), data_formatada: formatarDataBR(v.timestamp), id: v._id }));
...
app.post('/admin/api/cambistas/criar', checkAdmin, async (req, res) => {
    ...
    } catch(e) { res.status(400).json({ success: false, message: 'Erro ou usuário já existe' }); }
});
...
app.post('/admin/api/cambistas/adicionar-creditos', checkAdmin, async (req, res) => {
    ...
    await TransacaoCredito.create({
        cambista_id: cambistaId, admin_usuario: req.session.usuario, valor_alteracao: parseFloat(valor), tipo: 'recarga'
    });
...
app.get('/admin/api/comissoes', checkAdmin, async (req, res) => {
    const comissoes = await Comissao.find().populate('cambista_id', 'usuario').populate('venda_id', 'nome_jogador').sort({ timestamp: -1 });
    const formatadas = comissoes.map(c => ({
        id: c._id, valor_venda: c.valor_venda, valor_comissao: c.valor_comissao, status_pagamento: c.status_pagamento,
        data_formatada: formatarDataBR(c.timestamp), nome_jogador: c.venda_id ? c.venda_id.nome_jogador : 'Desconhecido',
        cambista_usuario: c.cambista_id ? c.cambista_id.usuario : 'Removido'
    }));
    const pendentes = await Comissao.aggregate([{ $match: { status_pagamento: 'pendente' } }, { $group: { _id: null, total: { $sum: '$valor_comissao' } } }]);
...
// --- ROTAS DO CAMBISTA (MIGRADO PARA MONGO) ---
...
app.post('/cambista/gerar-cartelas', checkCambista, async (req, res) => {
    ...
    const novaVenda = await Venda.create({
        sorteio_id: sorteioAlvo, nome_jogador: nome, telefone: telefone, quantidade_cartelas: quantidade,
        valor_total: total, tipo_venda: 'Cambista', cartelas_json: JSON.stringify(cartelas), cambista_id: cambistaId, tipo_sorteio: 'regular'
    });
    
    await TransacaoCredito.create({
        cambista_id: cambistaId, admin_usuario: req.session.cambistaUsuario, valor_alteracao: -total, tipo: 'venda', venda_id: novaVenda._id
    });
...
app.get('/cambista/minhas-comissoes', checkCambista, async (req, res) => {
    const comissoes = await Comissao.find({ cambista_id: req.session.cambistaId }).populate('venda_id', 'nome_jogador').sort({ timestamp: -1 });
    const formatadas = comissoes.map(c => ({ ...c.toObject(), data_formatada: formatarDataBR(c.timestamp), nome_jogador: c.venda_id ? c.venda_id.nome_jogador : '?' }));
    const pendentes = await Comissao.aggregate([{ $match: { cambista_id: new mongoose.Types.ObjectId(req.session.cambistaId), status_pagamento: 'pendente' } }, { $group: { _id: null, total: { $sum: '$valor_comissao' } } }]);
    res.json({ success: true, comissoes: formatadas, totalPendente: pendentes[0] ? pendentes[0].total : 0 });
});
...
async function getAdminStatusData() {
    ...
    const statusData = { estado: estadoJogo, sorteioAtual: idSorteioAtual, tempoRestante: (estadoJogo === 'ESPERANDO') ? tempoRestante : null, jogadoresReais: getContagemJogadores().reais, vendasProximoSorteio: { qtd_cartelas: 0, valor_total: 0 }, receitaDoDia: 0, proximoSorteioId: 0 };
    try {
        ...
        const vendasRes = await Venda.aggregate([ { $match: matchCriteria }, { $group: { _id: null, qtd: { $sum: 1 }, total: { $sum: "$valor_total" } } } ]);
        if (vendasRes.length > 0) statusData.vendasProximoSorteio = { qtd_cartelas: vendasRes[0].qtd, valor_total: vendasRes[0].total };
        const startOfDay = new Date(); startOfDay.setHours(0, 0, 0, 0);
        const endOfDay = new Date(); endOfDay.setHours(23, 59, 59, 999);
        const receitaRes = await Venda.aggregate([ { $match: { timestamp: { $gte: startOfDay, $lte: endOfDay } } }, { $group: { _id: null, total: { $sum: "$valor_total" } } } ]);
        if (receitaRes.length > 0) statusData.receitaDoDia = receitaRes[0].total;
    } catch (error) { console.error("Erro status admin:", error); }
    return statusData;
}
        ...
        for(const sid in jogadores) {
            ...
            for(let i=0; i<jog.cartelas.length; i++) {
                ...
                if(checarVencedorCartelaCheia(jog.cartelas[i], sSet)) {
                    ...
                    setTimeout(() => {
                        const premioValor = sorteioEspecialEmAndamento ? parseFloat(String(Config.findOne({chave:'sorteio_especial_valor'}).valor || '1000')) : PREMIO_CHEIA;
...
io.on('connection', async (socket) => {
    ...
    socket.emit('estadoInicial', {
        sorteioId: idAtual, estado: estadoJogo, tempoRestante: (estadoJogo==='ESPERANDO' && !sorteioEspecialEmAndamento) ? tempoRestante : 0,
        jogadoresOnline: cont.total, jogadoresReais: cont.reais, ultimosVencedores: ultimos,
        numerosSorteados: numerosSorteados, configuracoes: configMap
    });

    socket.on('criarPagamento', async (dados, cb) => {
        const { nome, telefone, quantidade, refCode } = dados;
        let cambistaIdParaSalvar = null;
        if (refCode) { const c = await Cambista.findOne({ usuario: refCode, ativo: true }); if (c) cambistaIdParaSalvar = c._id; }
        try {
            if(!process.env.BASE_URL) return cb({success:false, message:'URL base não configurada'});
            const preco = parseFloat(PRECO_CARTELA); const total = quantidade * preco;
            const payment = new Payment(mpClient);
            const body = { transaction_amount: total, description: `Compra ${quantidade} cartelas`, payment_method_id: 'pix', notification_url: `${process.env.BASE_URL}/webhook-mercadopago`, payer: { email: `jog_${telefone}@bingo.com`, first_name: nome } };
            const response = await payment.create({ body });
            const pid = response.id.toString();
            dados.tipo_compra = 'regular';
            await PagamentoPendente.updateOne({ payment_id: pid }, { socket_id: socket.id, dados_compra_json: JSON.stringify(dados), cambista_id: cambistaIdParaSalvar }, { upsert: true });
    ...
    socket.on('criarPagamentoEspecial', async (dados, cb) => {
        ...
        try {
            if(!process.env.BASE_URL) return cb({success:false, message:'URL base não configurada'});
            const preco = parseFloat(PRECO_CARTELA_ESPECIAL_ATUAL); const total = quantidade * preco;
            const payment = new Payment(mpClient);
            const body = { transaction_amount: total, description: `Especial ${quantidade} cartelas`, payment_method_id: 'pix', notification_url: `${process.env.BASE_URL}/webhook-mercadopago`, payer: { email: `jog_${telefone}@bingo.com`, first_name: nome } };
            const response = await payment.create({ body });
            const pid = response.id.toString();
    ...
    socket.on('criarDeposito', async(dados, cb) => {
        const { valor, jogadorId } = dados;
        try {
            const jogador = await Jogador.findById(jogadorId);
            if(!jogador) return cb({success:false, message: 'Jogador não encontrado'});
            const pay = new Payment(mpClient);
            const body = { transaction_amount: parseFloat(valor), description: `Deposito Carteira`, payment_method_id: 'pix', notification_url: `${process.env.BASE_URL}/webhook-mercadopago`, payer: { email: `wallet_${jogador.telefone}@bingo.com`, first_name: jogador.nome } };
            const r = await pay.create({ body });
            const pid = r.id.toString();
            const dadosMeta = { tipo_transacao: 'deposito_carteira', jogador_id: jogadorId, valor: valor };
            await PagamentoPendente.create({ payment_id: pid, socket_id: socket.id, dados_compra_json: JSON.stringify(dadosMeta) });
            cb({success:true, qrCodeBase64:r.point_of_interaction.transaction_data.qr_code_base64, qrCodeCopiaCola:r.point_of_interaction.transaction_data.qr_code, paymentId:pid});
        } catch(e) { console.error(e); cb({success:false}); }
    });

    socket.on('comprarComSaldo', async(dados, cb) => {
        const { jogadorId, quantidade, tipo } = dados; 
        try {
            const jogador = await Jogador.findById(jogadorId);
            if(!jogador) return cb({success:false, message: 'Sessão inválida. Faça login.'});
            let precoUnitario = parseFloat(PRECO_CARTELA);
            if(tipo === 'especial') precoUnitario = parseFloat(PRECO_CARTELA_ESPECIAL_ATUAL);
            const total = quantidade * precoUnitario;
            if(jogador.saldo < total) return cb({success:false, message: 'Saldo insuficiente. Faça um depósito.'});
            
            jogador.saldo -= total; await jogador.save();
            await TransacaoJogador.create({ jogador_id: jogador._id, tipo: 'compra', valor: -total, descricao: `Compra ${quantidade} cartelas (${tipo})` });
            
            let cartelasGeradas = []; let sorteioAlvo = 0; let tipoSorteioDB = 'regular'; let sorteioEspecialID = null;
            if(tipo === 'especial') { sorteioAlvo = 0; tipoSorteioDB = 'especial_agendado'; sorteioEspecialID = SORTEIO_ESPECIAL_DATAHORA; for(let i=0; i<quantidade; i++) cartelasGeradas.push(gerarDadosCartela(SORTEIO_ESPECIAL_DATAHORA)); } 
            else { sorteioAlvo = (estadoJogo === "ESPERANDO") ? numeroDoSorteio : numeroDoSorteio + 1; for(let i=0; i<quantidade; i++) cartelasGeradas.push(gerarDadosCartela(sorteioAlvo)); }
            
            const venda = await Venda.create({ sorteio_id: sorteioAlvo, nome_jogador: jogador.nome, telefone: jogador.telefone, quantidade_cartelas: quantidade, valor_total: total, tipo_venda: 'Saldo', cartelas_json: JSON.stringify(cartelasGeradas), tipo_sorteio: tipoSorteioDB, sorteio_id_especial: sorteioEspecialID, jogador_id: jogador._id });
            if(tipo === 'regular') { const pid = `saldo_${gerarIdUnico()}`; jogadores[pid] = { nome: jogador.nome, telefone: jogador.telefone, isManual: true, cartelas: cartelasGeradas }; io.emit('contagemJogadores', getContagemJogadores()); }
            cb({success:true, novoSaldo: jogador.saldo, vendaId: venda._id});
        } catch(e) { console.error(e); cb({success:false, message: 'Erro ao processar compra.'}); }
    });
    ...
    socket.on('buscarCartelasPorTelefone', async (data, cb) => {
        const vendas = await Venda.find({ telefone: data.telefone, $or: [ { tipo_sorteio: 'especial_agendado' }, { tipo_sorteio: 'regular', sorteio_id: { $gte: numeroDoSorteio - 5 } } ] }).sort({ timestamp: -1 }).limit(20);
        const formatadas = vendas.map(v => ({ id: v.id, sorteio_id: v.sorteio_id, sorteio_id_especial: v.sorteio_id_especial, quantidade_cartelas: v.quantidade_cartelas, data_formatada: formatarDataBR(v.timestamp), nome_jogador: v.nome_jogador, tipo_sorteio: v.tipo_sorteio }));
    ...
    socket.on('checarMeusPremios', async (data, cb) => {
        ...
        if(formatados.length > 0) cb({ success: true, premios: formatados }); else cb({ success: false });
    });
    
    socket.on('buscarMinhasCartelas', async (data) => {
        try { const v = await Venda.findById(data.vendaId); if(v && (v.nome_jogador === data.nome || v.jogador_id)) socket.emit('cartelasEncontradas', { cartelas: JSON.parse(v.cartelas_json) }); else socket.emit('cartelasNaoEncontradas'); } catch(e) { socket.emit('cartelasNaoEncontradas'); }
    });

    socket.on('registerPlayer', (p) => {
        if(p && p.cartelas.length > 0) {
            const sId = p.cartelas[0].s_id; const valido = sorteioEspecialEmAndamento ? SORTEIO_ESPECIAL_DATAHORA : numeroDoSorteio;
            if(sId == valido) { jogadores[socket.id] = { nome: p.nome, telefone: p.telefone, cartelas: p.cartelas }; io.emit('contagemJogadores', getContagemJogadores()); } else socket.emit('cartelaAntiga');
        }
    });
    
    socket.on('getAdminStatus', async () => {
        try { const statusData = await getAdminStatusData(); socket.emit('adminStatusUpdate', statusData); } catch (e) { console.error("Erro ao processar getAdminStatus:", e); }
    });
