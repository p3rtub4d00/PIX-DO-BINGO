                ...
                if (pag.status === 'approved') {
                    ...
                    if (pendente) {
                        ...
                        if (dadosCompra.tipo_compra === 'especial') {
                            const preco = parseFloat(PRECO_CARTELA_ESPECIAL_ATUAL);                            valorTotal = dadosCompra.quantidade * preco;
...
// --- INICIO DA LÓGICA DE 2000 NOMES DE BOTS MISTOS (PESSOAS E BARES) ---
const primeirosNomes = [
    "Ana", "Bruno", "Carlos", "Daniela", "Eduardo", "Fernanda", "Gabriel", "Helena", "Igor", "Juliana",
    "Lucas", "Mariana", "Nicolas", "Olivia", "Paulo", "Rafael", "Sabrina", "Tiago", "Vanessa", "Wagner",
    "Adriana", "Beto", "Caio", "Debora", "Elias", "Fabio", "Gisele", "Hugo", "Isabela", "Jorge",
...
const sobrenomes = [
    ...
    "Cardoso", "Ramos", "Goncalves", "Santana", "Teixeira", "Araujo", "Cavalcanti", "Pinto", "Maia", "Campos",
    "Sales", "Farias", "Barros", "Moraes", "Batista", "Neves", "Siqueira", "Braga", "Peixoto", "Macedo"
];

const tiposBar = [
    "Bar do", "Boteco do", "Espetinho do", "Adega do", "Cantinho do", "Quiosque do", "Bar da", "Lanchonete da", "Point do", "Toca do"
];

const apelidosBar = [
    "Zé", "Gordo", "Baixinho", "Careca", "Alemao", "Japa", "Bigode", "Ceara", "Bahia", "Mineiro",
    "Chico", "Toninho", "Carlão", "Dona Maria", "Tia Juju", "Vovo", "Amigos", "Encontro", "Sossego", "Pescador"
];

function gerarListaMista(qtdAlvo) {
    const listaFinal = new Set(); // Set garante que não haja duplicatas
    
    // Tenta gerar até atingir a quantidade ou um limite de tentativas de segurança
    let tentativas = 0;
    while(listaFinal.size < qtdAlvo && tentativas < qtdAlvo * 5) {
        tentativas++;
        const tipo = Math.random(); // Decide se gera Nome ou Bar

        if (tipo < 0.85) { 
            // 85% de chance de ser Pessoa
            const nome = primeirosNomes[Math.floor(Math.random() * primeirosNomes.length)];
            const sobre = sobrenomes[Math.floor(Math.random() * sobrenomes.length)];
            listaFinal.add(`${nome} ${sobre}`);
        } else {
            // 15% de chance de ser Bar
            const prefixo = tiposBar[Math.floor(Math.random() * tiposBar.length)];
            
            // Pode ser "Bar do Zé" ou "Bar do Silva" (usando sobrenome)
            let sufixo;
            if (Math.random() > 0.5) {
                sufixo = apelidosBar[Math.floor(Math.random() * apelidosBar.length)];
            } else {
                sufixo = primeirosNomes[Math.floor(Math.random() * primeirosNomes.length)];
            }
            
            listaFinal.add(`${prefixo} ${sufixo}`);
        }
    }
    return Array.from(listaFinal);
}

// Gera 2000 nomes automaticamente ao iniciar
const nomesBots = gerarListaMista(2000);
...
function escolherJogadorRealParaPremioLinha(sorteioId, sorteadosAtuais) {
    ...
    const jogadorEscolhido = candidatos[randomInt(0, candidatos.length)];
    const cartelaEscolhida = jogadorEscolhido.cartelasElegiveis[randomInt(0, jogadorEscolhido.cartelasElegiveis.length)];

    return {
        sid: jogadorEscolhido.sid,
        nome: jogadorEscolhido.jog.nome,
        telefone: jogadorEscolhido.jog.telefone,
        cartela: cartelaEscolhida.cartela,
        indiceCartela: cartelaEscolhida.indice,
        isManual: jogadorEscolhido.jog.isManual
    };
}
...
async function liberarPremioLinhaAutomaticoPorLucro(idSorteio) {
    ...
    if (sock && !vencedorReal.isManual) {
        sock.emit('voceGanhouLinha', {
            cartelaGanhadora: vencedorReal.cartela,
            indiceCartela: vencedorReal.indiceCartela,
            premioValor: PREMIO_LINHA
        });
        sock.broadcast.emit('alguemGanhouLinha', { nome: vencedorReal.nome });
    } else {
        io.emit('alguemGanhouLinha', { nome: vencedorReal.nome });
    }
...
async function getAdminStatusData() {
    ...
    } catch (error) {        console.error("Erro ao calcular status do admin:", error);
    }
...
async function terminarRodada(vencedor, socketId) {
    ...
    if(vencedor) {
        if(socketId && io.sockets.sockets.get(socketId)) {
            io.to(socketId).emit('voceGanhouCartelaCheia', vencedor);            io.sockets.sockets.get(socketId).broadcast.emit('alguemGanhouCartelaCheia', { nome: vencedor.nome });
        } else {
...
io.on('connection', async (socket) => {
    ...
    socket.on('buscarMinhasCartelas', async (data = {}) => {
        try {
            ...
            const nomeReq = String(data.nome || '').trim().toLowerCase();
            const nomeVenda = String(v.nome_jogador || '').trim().toLowerCase();
            const telReq = String(data.telefone || '').replace(/\D/g, '');
            const telVenda = String(v.telefone || '').replace(/\D/g, '');
